...
function exportTestsToCSV(tests) {
  ...
  tests.forEach((test, idx) => {
    test.valores.forEach((valor, i) => {
      rows += `${idx + 1},${i + 1},${typeof valor === 'object' ? (valor.fuerza1 !== undefined ? valor.fuerza1 : valor.distancia) : valor},"${test.nombre || ''}"\n`;
    });
  });
...
export default function App() {
  ...
  useEffect(() => {
    ...
    if (window.electronAPI) {
      window.electronAPI.onBLEDevice((device) => {
        console.log('Dispositivo BLE:', device);
      });
    }
